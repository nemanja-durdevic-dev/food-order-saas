...
export async function toggleRecordField(
  ...
  if (!resource?.toggleField) {
    return { error: "This collection does not support toggling." };
  }

  const membership = await requireAdminMembership().catch(() => null);
  ...
  const { data: current, error: fetchError } = await supabaseAdmin
    ...
    .select(key)
    .eq("id", id)
    .eq(scopeColumn, membership.restaurant_id)
    .maybeSingle();
  ...
  if (trueValue !== undefined && falseValue !== undefined) {
    newValue = currentValue === trueValue ? falseValue : trueValue;
  } else {
