...
import { createPayment, isNetsConfigured } from "@/lib/nets";
...
export async function POST(request: NextRequest) {
  ...
  if (!isNetsConfigured) {
    return NextResponse.json({ error: "NETS not configured" }, { status: 500 });
  }
  ...
  try {
    ...
    const netsItems = body.cartItems.map((item: CartItemInput) => {
      const netTotal = Math.round(item.unitPrice * 100 * item.quantity);
      const grossTotal = Math.round(item.total * 100);
    ...
    const netsPayment = await createPayment({
      ...
      webhookUrl: `${origin}/api/webhook/nets`,
    });

    await supabaseAdmin
      .from("orders")
      .update({ nets_payment_id: netsPayment.paymentId })
      .eq("id", order.id);

    return NextResponse.json({ url: netsPayment.hostedPaymentPageUrl, orderId: order.id });
  } catch (err) {
    console.error("NETS checkout error:", err);
    return NextResponse.json({ error: "NETS checkout failed" }, { status: 500 });
  }
}
